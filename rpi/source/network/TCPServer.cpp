...
TCPServer::TCPServer(int port)
{
    ...
    if (bind(listen_fd, (struct sockaddr*)&listen_addr, sizeof(listenaddr)) == -1) // master socket 커널과 연결
    {
        perror("bind failed");
        exit(-1);
    }
...
ControlFrame TCPServer::receiveCommand()
{
    struct ControlFrame control_msg;
    ssize_t received = recv(data_socket, &control_msg, sizeof(control_msg), 0);
    if (received <= 0)
    {
        std::cerr << "Connection Lost!" << endl;
        exit(-1);
    }
    return control_msg;
}
